...
volatile unsigned int phase[MAX_RESULTS];
volatile unsigned int I_data[MAX_RESULTS];
volatile unsigned int Q_data[MAX_RESULTS];
...
inline int16_t ssb(int16_t in)
{
  ...
  I_data[resultNumber]=i;
  Q_data[resultNumber]=q;
...
ISR (ADC_vect)
  ...
  if (resultNumber >= MAX_RESULTS)
    ...
    //t0=micros();
    phase[resultNumber]=ssb(results[resultNumber]);
    //stamp[resultNumber]=micros()-t0;
    resultNumber++;
 ...
 *   Initialize AD9850
 */
  sprintf(hi,"%s Version %s",PROGRAMID,VERSION);
  lcd.print(hi);
    
  for (int i = 0; i < MAX_RESULTS; i++) {
    sprintf(hi,"%d,%d,%d,%d,%d\n",i,results[i],phase[i],I_data[i],Q_data[i]);       
    Serial.print(hi);
